...
ulsan_humid = [build_xgb(params) for n_in in n_in_list]
dangjin_humid = [build_xgb(params) for n_in in n_in_list]

# parameter tunning by gridsearch
for n_in, model1, model2 in zip(n_in_list, ulsan_humid, dangjin_humid):
    (x_train, y_train), (x_test, y_test) = data_generator(ulsan_data, n_in, n_out, ratio, ["humid_obs"], ["humid_obs"])
    model1.fit(x_train, y_train, eval_set = [(x_test, y_test)], early_stopping_rounds = esr)

    (x_train, y_train), (x_test, y_test) = data_generator(dangjin_data, n_in, n_out, ratio, ["humid_obs"], ["humid_obs"])
    model2.fit(x_train, y_train, eval_set = [(x_test, y_test)], early_stopping_rounds = esr)
...
actual = ulsan_data["humid_obs"][-24*30*1 : ].values
