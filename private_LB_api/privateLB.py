...
class API:
    def __init__(self, base_date, base_time, location, key):
        ...
        predict_date = pd.Timestamp(base_date) + pd.Timedelta(days=1)

        self.predict_date_json = predict_date.strftime("%Y%m%d")
        self.predict_date_pandas = predict_date.strftime("%Y-%m-%d")

        predict_date_json_tmrw = predict_date + pd.Timedelta(days=1)
        self.predict_date_json_tmrw = predict_date_json_tmrw.strftime("%Y%m%d")
    ...
    def get_data(self):
        ...
        for i, data in enumerate(response["response"]["body"]["items"]["item"]):
            if (data["fcstDate"] == self.predict_date_json) or (
                (data["fcstDate"] == self.predict_date_json_tmrw)
